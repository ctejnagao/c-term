...
export async function POST() {
  try {
    ...
    // 3. Load 資料/MEISAI20250829151403.csv
    ...
    if (fs.existsSync(csvPath)) {
      ...
      for (const line of lines) {
        ...
        if (description.includes('ラクテン') || description.includes('楽天')) {
    ...
    // 4. Seed SMBC (上前津支店 - 借入金返済専用口座) initial records from 資料/銀行残高2025.xlsx
    const smbcEntries = [
      {
    ...
    return NextResponse.json({
      success: true,
      message: `資料データ（UFJ明細${importedCount}件、楽天カード明細3期分、自動仕訳ルール）の投入が完了しました`,
      importedCount,
    });
  } catch (error: any) {
    console.error('Failed to seed bank transactions:', error);
    return NextResponse.json({ error: error.message || '初期データ投入に失敗しました' }, { status: 500 });
  }
}
